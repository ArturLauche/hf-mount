...
pub use crate::context::{RpcAuthRequest, RpcAuthorizer};
pub use crate::{auth_flavor, auth_unix};
...
pub struct NFSTcpListener<T: NFSFileSystem + Send + Sync + 'static> {
    ...
    authorizer: Option<Arc<dyn RpcAuthorizer>>,
...
impl<T: NFSFileSystem + Send + Sync + 'static> NFSTcpListener<T> {
    ...
    pub fn with_authorizer(&mut self, authorizer: Arc<dyn RpcAuthorizer>) {
    ...
    async fn handle_forever(&self) -> io::Result<()> {
        loop {
            ...
            let context = RPCContext {
                ...
                auth_flavor: crate::auth_flavor::AUTH_NULL,
                auth: crate::auth_unix::default(),
