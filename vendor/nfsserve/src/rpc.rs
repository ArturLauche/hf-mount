...
impl auth_unix {
    pub fn new(uid: u32, gid: u32, gids: Vec<u32>) -> Self {
        Self {
            stamp: 0,
            machinename: b"localhost".to_vec(),
            uid,
            gid,
            gids,
        }
...
pub fn auth_error_reply_message(xid: u32, status: auth_stat) -> rpc_msg {
    let reply = reply_body::MSG_DENIED(rejected_reply::AUTH_ERROR(status));
    rpc_msg {
        xid,
        body: rpc_body::REPLY(reply),
    }
